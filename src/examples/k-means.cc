...
struct KMeans {
  ...
  void run(Master* m, const ConfigData& conf) {
    ...
    m->run(points,
        [](TableT<int32_t, Point>* points) {
          const int numShards = points->numShards;
          for (int64_t i = points->shard; i < FLAGS_num_points; i += numShards) {
    ...
    for (int i = 0; i < 10; ++i) {
      m->map(points, [](const int32_t& key, Point& p) {
        p.min_dist = 2;
        for (int i = 0; i < FLAGS_num_clusters; ++i) {
      ...
      m->map(clusters, [](const int32_t& key, Cluster& c) {
        if (c.x == 0 && c.y == 0) {
      ...
      m->map(points, [](const int32_t key, const Point& p) {
